...
app.layout = html.Div([
    ...
    html.H1("Automobile Statistics Dashboard", style={'textAlign':'center', 'font-size':24, 'color':'#503D36'}),#Include style for title
    #TASK 2.2: Add two dropdown menus
    html.Div([html.Div([
       html.Label("Select Statistics:", style={'width':'125px'}),
       dcc.Dropdown(
            id='dropdown-statistics',
            options=dropdown_options,
            value='Select Statistics',
            placeholder='Select a report type',
            style={'width':'150px', 'padding':'1px', 'font-size':'15px', 'height':'30px'}
        )
    ],style={'display':'flex', 'gap':'10px'}),
    html.Div([
        html.Label("Select Year: ", style={'width':'125px'}),
        dcc.Dropdown(
            id='select-year',
    ...
    ],style={'display':'flex', 'gap':'10px'})], style={'display': 'flex', 'gap':'50px'})
    ,
...
#Plot 1 Automobile sales fluctuate over Recession Period (year wise)
        ...
        width=600,  # Set width (adjust as needed)
...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        ...
        R_chart2  = dcc.Graph(
            ...
            x='Vehicle_Type',
            y='Automobile_Sales',
            title="Average number of Vehicles Sold by Vehicle type").update_layout(
        width=600,  # Set width (adjust as needed)
        height=300  # Set height (adjust as needed)
    ))
	...
	# Hint:Use Vehicle_Type and Advertising_Expenditure columns
        ...
        R_chart3 = dcc.Graph(
            figure=px.pie(exp_rec,
                          ...
                          title = "Advertising Expenditure shared by Vehicle type during Recessions"
                          ).update_layout(
        width=600,  # Set width (adjust as needed)
        height=300  # Set height (adjust as needed)
    ))
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
	...
	# Hint:Use unemployment_rate,Vehicle_Type and Automobile_Sales columns
        ...
        title='Effect of Unemployment Rate on Vehicle Type and Sales').update_layout(
        width=600,  # Set width (adjust as needed)
        height=300  # Set height (adjust as needed)
    ))
